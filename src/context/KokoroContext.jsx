...
export function KokoroProvider({ children }) {
  ...
  const loadPipeline = useCallback(async () => {
    ...
    const progress_callback = (p) => {
      if (p.status === 'initiate') {
        ...
        useKokoroStore.getState().setCurrentFile(p.file?.split('/').pop() ?? '');
      }
      ...
      if (p.status === 'progress') {
        ...
        useKokoroStore.getState().setCurrentFile(p.file?.split('/').pop() ?? '');
        useKokoroStore.getState().setProgress(totalSize > 0 ? Math.round((totalLoaded / totalSize) * 100) : (p.progress ?? 0));
      }
  ...
  const speakSingle = useCallback(async (text, speed = 1.0) => {
    if (!text) return;
    ...
    try {
      ...
      if (!tts) { useKokoroStore.getState().setSpeaking(false); return; }
  ...
  const speakMultiple = useCallback(async (texts, speed = 1.0) => {
    ...
    try {
      ...
      const checkDone = () => {
        if (streamDone && finishedSources >= totalSources && audioCtxRef.current === ctx) {
          useKokoroStore.getState().setSpeaking(false);
          ctx.close();
          audioCtxRef.current = null;
        }
  ...
  const speak = useCallback((textOrTexts, speed = 1.0) => {
  ...
  return (
    <KokoroContext.Provider value={{ speak, stop, preload: loadPipeline, selectDevice }}>
      {children}

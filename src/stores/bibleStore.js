...
export const useBibleStore = create(
  ...
  fetchVerse: async (passage) => {
    ...
    if (hit) {
      set({ verse: hit.text, verses: hit.verses ?? [], reference: hit.reference, loading: false, error: null }, false, 'fetchVerse/cache');
      return;
    }
    ...
    try {
      ...
      const res  = await globalThis.fetch(`/api/bible?passage=${encodeURIComponent(passage)}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? `Request failed (${res.status})`);
      if (key !== 'random') sessionCache.set(key, data);
      set({ verse: data.text, verses: data.verses ?? [], reference: data.reference, loading: false }, false, 'fetchVerse/done');
    } catch (err) {
      set({ error: err.message ?? 'Could not load verse. Please try again.', loading: false }, false, 'fetchVerse/error');
    }
  },
